...
<body>
	...
	<div class="container">
		...
		<!-- table header auto-generated : -->
					<th width="10%"><?= $this->lang->line('itineraire.form.itinuord.label') ?></a></th>
					<th><?= $this->lang->line('itineraire.form.itilbnom.label') ?></a></th>
					<th width="10%"><?= $this->lang->line('itineraire.form.itinudst.label') ?></a></th>
					<th width="10%"><?= $this->lang->line('itineraire.form.itilbdur.label') ?></a></th>
					<?php if($isAuteur) {?>
					<th width="10%"><?= $this->lang->line('object.tableheader.actions') ?></th>
					<?php } ?>
				</tr>
			</thead>
			<tbody>
...
?>
			<tr>
				...
				<td valign="top"><a href="<?=base_url()?>index.php/itineraire/detailitineraire/index/<?=$itineraire->itiiditi?>"><?=$itineraire->itilbnom?></a>
					<span class="help-block valtype"><?=$itineraire->ititxdes?></span>
				</td>
				</td>
...
endforeach; ?>
		...
		?>
				<li>
					...
					<b><?= $sharedUser->usrlbnom ?></b> <br/>
					<a href="mailto:<?= $sharedUser->usrlbmai ?>">
					...
					<a class="" href="#" onclick="if( confirm('<?= $this->lang->line('partage.message.askConfirm.deletion')?>')){document.location.href='<?=base_url()?>index.php/partage/listpartages/deleteUsrVoy?paridami=<?=$sharedUser->usridusr?>&paridvoy=<?= $voyage->voyidvoy ?>';}" 
						title="<?= $this->lang->line('form.button.delete') ?>"
						><i class="glyphicon glyphicon-remove"> </i><?= $this->lang->line('form.button.delete') ?></a></td>
				</li>
		<?php 
		endforeach; ?>
			</ul>
