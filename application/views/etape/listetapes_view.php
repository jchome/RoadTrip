...
<body>
	...
	<div class="container">
		...
		<!-- table header auto-generated : -->
					...
					<th class="sortable"><!-- etptxdes -->
						<a href="<?=base_url()?>index.php/etape/listetapes/index/etptxdes/<?= ($orderBy == 'etptxdes'&& $asc == 'asc')?('desc'):('asc') ?>"
						<?php if($orderBy == 'etptxdes'&& $asc == 'asc') {?>
							class=" sortAsc"
						<?php }else if($orderBy == 'etptxdes'&& $asc == 'desc') {?>
							class=" sortDesc"
						<?php }?>
						><?= $this->lang->line('etape.form.etptxdes.label') ?></a></th>
...
?>
	<tr>
				...
				<td valign="top"><?=($etape->etpiditi == 0)?(""):($itineraireCollection[$etape->etpiditi]->itilbnom)?>
			</td>
				<td valign="top"><?=$etape->etpnuord?></td>
				<td valign="top"><?=$etape->etptxdes?></td>
