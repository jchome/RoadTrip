...
foreach($etapesCollection as $etape){
	echo "points[". $etape->etpnuord ."] = { titre: \"" . str_replace('"','\\"',$etape->etplbnom) . "\", \n";
	echo "  ordre: ".$etape->etpnuord.",\n";
	echo "  id: ".$etape->etpidetp.",\n";
	echo "  lat: ".$etape->etpnulat . ", lon:". $etape->etpnulon."\n";
	echo "};\n";
}?>
...
<body role="document">
	...
	<div class="container">
		...
		<h2><?= $this->lang->line('detail_itineraire.form.title', $itineraire->itilbnom) ?>
		...
		<?php if($itineraire->ititxdes != "" ){?>
		<p><?= $itineraire->ititxdes ?></p>
		<?php } ?>
			<?php
		...
		<!-- table header auto-generated : -->
					<th width="10%"><?= $this->lang->line('etape.form.etpnuord.label') ?></th>
					<th><?= $this->lang->line('etape.form.etplbnom.label') ?></th>
					<?php if($isAuteur) { ?>
					<th width="15%"><?= $this->lang->line('etape.form.move') ?></th>
					<th width="15%"><?= $this->lang->line('object.tableheader.actions') ?></th>
					<?php } ?>
				</tr>
			</thead>
			<tbody>
...
?>
			<tr>
				...
				<td valign="top"><?=$etape->etplbnom?>
				<?php if($etape->etptxdes != ""){ ?>
					<br/><div class="small" style="color: #777;"><i><?= $etape->etptxdes ?></i></div>
				<?php } ?>
				</td>
				<?php if($isAuteur) { ?>
				<td>
