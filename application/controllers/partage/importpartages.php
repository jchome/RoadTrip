...
class ImportPartages extends CI_Controller {
	...
	public function index(){
		$data = array();
		$this->load->view('partage/importpartages_view', $data);
	}
	 ...
	 * @return multitype:Partage_model
	 */
	private function parseFile($filename, $separator){
		...
		if (($handle = fopen($filename, "r")) !== FALSE) {
			while (($data = fgetcsv($handle, $maxSize, $separator)) !== FALSE) {
				...
				$model->paridvoy = $data[0];
				$model->paridami = $data[1];	
				$model->save($this->db);
