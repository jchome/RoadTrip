...
class ImportItineraires extends CI_Controller {
	...
	public function index(){
		$data = array();
		$this->load->view('itineraire/importitineraires_view', $data);
	}
	 ...
	 * @return multitype:Itineraire_model
	 */
	private function parseFile($filename, $separator){
		...
		if (($handle = fopen($filename, "r")) !== FALSE) {
			while (($data = fgetcsv($handle, $maxSize, $separator)) !== FALSE) {
				...
				$model->idilbnom = $data[0];
				$model->itiidvoy = $data[1];
				$model->itinuord = $data[2];
				$model->itilbdur = $data[3];
				$model->itinudst = $data[4];	
				$model->save($this->db);
